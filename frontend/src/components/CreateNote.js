        ...
        try {
            ...
            if (usersResponse.data.length > 0) {
                this.setState({
                    ...
                    userSelected: usersResponse.data[0].username
                });
            }
    
            if (this.props.match.params.id) {
                const notesResponse = await axios.get('http://localhost:4000/api/notes/' + this.props.match.params.id);
    
                this.setState({
                    title: notesResponse.data.title,
                    content: notesResponse.data.content,
                    date: new Date(notesResponse.data.date),
                    userSelected: notesResponse.data.author,
                    editing: true,
